...
Adapted for bakery-lens package (using numpy arrays directly instead of Frame entity).
"""
...
class TestFocusLensConfig:
    ...
    def test_invalid_focus_size_not_multiple_of_32(self):
        """
        Scenario: Reject focus_size not multiple of 32
            Given focus_size=650 (not multiple of 32)
            When creating FocusLensConfig
    ...
    def test_all_valid_multiples_of_32(self):
        """
        Scenario: Accept all valid multiples of 32
            Given various multiples of 32
            When creating FocusLensConfig
            Then all should succeed
        """
        valid_sizes = [32, 64, 96, 128, 160, 320, 640]
...
class TestApplyFocusLens:
    ...
    def test_crop_centered(self):
        ...
        cropped_data, crop_info = apply_focus_lens(frame_data, 0, config)

        h, w = cropped_data.shape[:2]
        assert w == 640
        assert h == 640
    ...
    def test_pad_strategy_small_frame(self):
        ...
        assert crop_info.pad_x == 240 # 640 - 400
        assert crop_info.pad_y == 240
...
class TestMapDetectionsToFullFrame:
    ...
    def test_pad_strategy_detections(self):
        """
        Scenario: Correctly map detections when pad strategy was used
            Given crop with pad_x=240, scale=1.0
            When mapping to full frame
            Then padding should be subtracted
        """
        # Original: 400x400. Focus: 640. Pad: x=240, y=240.
        # Pad left = 120, Pad top = 120
        crop_info = CropInfo(x=0, y=0, width=640, height=640, pad_x=240, pad_y=240)
        
        # Detection at 130, 130 (inside the valid region starting at 120, 120)
        detections = sv.Detections(
            xyxy=np.array([[130, 130, 150, 150]]),
            confidence=np.array([0.9]),
            class_id=np.array([0])
        )
        
        mapped = map_detections_to_full_frame(detections, crop_info, 400, 400)
