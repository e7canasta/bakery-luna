...
**Related ADR:** [ADR-007: Exporters Architecture](/docs/adrs/007-exporters-architecture.md)
...
uv run scripts/tasks/export/export_sauron_segmentation.py \
  --model 26 --all --dry-run
```

#### export_sauron_pose.py

Export pose estimation models to FP16 (GPU).
...
The ONNX export step may have failed. Check:
1. Disk space
2. YOLO model name is correct
3. No permissions issues in models directory
...
```bash
# Test imports
uv run python -c "from bakery_exporters import ExportPipeline"
...
# Show plan only
uv run scripts/tasks/export/export_int8.py \
  --model 26 --model-size n --type segmentation --resolution 320 --dry-run
```

## Related

- **ADR:** [ADR-007: Exporters Architecture](/docs/adrs/007-exporters-architecture.md)
- **Code:** [bakery-exporters/](/bakery-exporters/)
- **Usage:** [USAGE.md](/bakery-exporters/USAGE.md)
