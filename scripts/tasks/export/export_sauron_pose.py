"""
Bakery - Sauron Eye Export Pipeline (Pose)
==========================================

Exporta modelos de pose estimation YOLO a formatos optimizados.
...
Uso:
    ...
    uv run export_sauron_pose.py --model 26 --model-size n --resolution 320

    # Exportar múltiples combinaciones
    uv run export_sauron_pose.py --model 26 --model-size n s m --resolution 256 320

    # Exportar todo
    uv run export_sauron_pose.py --model 26 --all

    # Dry-run
    uv run export_sauron_pose.py --model 26 --model-size n --resolution 320 --dry-run
...
def export_to_onnx(model_name: str, resolution: int) -> Path:
    ...
    Args:
        model_name: Full model name (e.g., yolo26n-pose)
...
def export_model(
    ...
    """
    model_name = ModelPath.get_model_name(yolo_version, size, "pose")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Export YOLO Pose models",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=f"""
Ejemplos:

  # YOLO11 (default)
  uv run export_sauron_pose.py --model-size m --resolution 256

  # YOLO26
  uv run export_sauron_pose.py --model 26 --model-size m --resolution 256
...
Configuración:
  ...
  BAKERY_ONNX_OPSET={config.onnx_opset}
    ...
    print("🎯 Bakery - Export Pipeline (Pose)")
    print("=" * 70)
